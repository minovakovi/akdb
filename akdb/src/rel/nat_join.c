    ...
    for (i = 0; i < DATA_BLOCK_SIZE;)
    {
        ...
        while (strcmp(temp_block->header[head].att_name, "") != 0)
        {
            ...
            while (some_element != NULL)
            {
                ...
                if ((strcmp(some_element->attribute_name, temp_block->header[head].att_name) == 0) && (size != 0) && (overflow < (temp_block->AK_free_space + 1)) && (overflow > -1))
                {
    ...
    for (i = 0; i < DATA_BLOCK_SIZE;)
    {
        ...
        while (strcmp(tbl1_temp_block->header[head].att_name, "") != 0)
        {
            ...
            while (list_elem != NULL)
            {
                ...
                if ((strcmp(list_elem->data, tbl1_temp_block->header[head].att_name) == 0) && (size != 0) && (overflow < (tbl1_temp_block->AK_free_space + 1)) && (overflow > -1))
                {
                    memset(data, '\0', MAX_VARCHAR_LENGTH);
