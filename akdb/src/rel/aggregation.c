...
int agg_task_from_string(const char *str)
{
	if (strcmp(str, "SUM") == 0)
		return AGG_TASK_SUM;
	if (strcmp(str, "AVG") == 0)
	...
	if (strcmp(str, "COUNT") == 0)
		return AGG_TASK_COUNT;
	if (strcmp(str, "MIN") == 0)
		return AGG_TASK_MIN;
	if (strcmp(str, "MAX") == 0)
		return AGG_TASK_MAX;
	if (strcmp(str, "GROUP") == 0)
		return AGG_TASK_GROUP;
	...
	while (addresses->address_from[i] != 0)
	{
		for (j = addresses->address_from[i]; j < addresses->address_to[i]; j++)
		{
			...
			for (k = 0; k < block->last_tuple_dict_id; k += student_numattr)
			{
				...
				{ // if that name is already added to first names array
					counts[added_name_index]++;
					sum_weights[added_name_index] += tmp_weight;
					if (tmp_weight < min_weights[added_name_index])
					{
						min_weights[added_name_index] = tmp_weight;
					}
					if (tmp_weight > max_weights[added_name_index])
					{
						max_weights[added_name_index] = tmp_weight;
					}
				}
				else
				{
		...
		while (el)
		{
			if (el->type != TYPE_AGGREGATOR)
			{
				...
				if (el->type == TYPE_ATTRIBS)
				{
					...
					int agg_task_code = agg_task_from_string(aggregator_node->data);
					AK_agg_input_add(t_header[AK_get_attr_index(source_table, el->data)], agg_task_code, input);
				}
			}
			el = el->next;
		}
...
TestResult AK_aggregation_having_test()
{
	...
	else
	{
		...
		int avg_task = AGG_TASK_AVG;
		AK_InsertAtEnd_L3(TYPE_AGGREGATOR, &avg_task, sizeof(int), having_expr);
		...
		int count_task = AGG_TASK_COUNT;
		AK_InsertAtEnd_L3(TYPE_AGGREGATOR, &count_task, sizeof(int), having_expr);
