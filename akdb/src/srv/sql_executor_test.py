...
# CREATE TABLE TEST
'''
create_table_output = executor.execute(
    "CREATE TABLE student (id_student INT, first_name VARCHAR, last_name VARCHAR, year_of_birth INT, grade_avg FLOAT)")
duplicate_create_output = executor.execute(
    "CREATE TABLE student (id INT, name VARCHAR, gpa FLOAT)")
...
sequence1_output = executor.execute("Create sequence numac_1 start with 1 increment by 2 minvalue 0 maxvalue 100 cycle")
sequence2_output = executor.execute("Create sequence numac_2 start with 1 increment by 2 minvalue 0 maxvalue 100")
def sequence_test():
    """
    >>> sequence1_output
    ('Create_sequence_command', 'Command succesfully executed')
    >>> sequence2_output
    ('Create_sequence_command', 'Command succesfully executed')
    """
...
trigger1_output = executor.execute("CREATE TRIGGER accept_conn AFTER INSERT ON conn FOR ROW EXECUTE PROCEDURE conn_accepted()")
trigger2_output = executor.execute("CREATE TRIGGER accept_conn BEFORE DELETE OR INSERT ON conn EXECUTE PROCEDURE conn_accepted()")
trigger3_output = executor.execute(
    "CREATE TRIGGER accept_conn BEFORE DELETE OR INSERT ON conn FOR EACH STATEMENT EXECUTE PROCEDURE conn_accepted(2,3,'data')")
trigger4_output = executor.execute(
...
# running this test causes SIGSEV and creates an infinite loop!!!
index1_output = executor.execute("CREATE INDEX Pindex ON student ( stupac1, stupac2 ) USING Btree")
index2_output = executor.execute("create index Pindex on student ( stupac1, stupac2 ) USING Hash")
...
def drop_test():
    """
    >>> executor.execute("DROP TABLE student") != False
    True
    """
