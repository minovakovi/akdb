...
def cleanup_db():
    ...
    global _db_initialized, _sql_executor_instance
    _db_initialized = False
    _sql_executor_instance = None
...
def safe_initialize_db():
    ...
    try:
        ...
        print("Initializing SQL executor database...")
        initialize()
        _db_initialized = True
        print("SQL executor database initialized successfully")
        return True
    except Exception as e:
        print(f"SQL executor database initialization failed: {e}")
        return False

def get_sql_executor_instance():
    """Get a single SQL executor instance"""
    global _sql_executor_instance
    if _sql_executor_instance is None:
        if safe_initialize_db():
            _sql_executor_instance = Sql_executor()
        else:
            print("Failed to initialize SQL executor database")
...
def get_attr_type_test():
    ...
    ['int', 'float', 'varchar', 'varchar', 'date', 'interval', 'boolean', 'datetime']
    """

# Setup variables at module level
sequence1_output = None
create_table_output = None
index1_output = None
index2_output = None
trigger1_output = None
trigger2_output = None
trigger3_output = None
trigger4_output = None
insert_into1_output = None
insert_into2_output = None
insert_into3_output = None
     
if __name__ == "__main__":
    executor = get_sql_executor_instance()
    
    # Add error handling to ensure executor is not None
    if executor is None:
        print("Failed to get SQL executor instance. Exiting.")
        sys.exit(1)
...
def create_index_test():  # Fixed typo: was create_index_text
    """
    >>> index1_output
    'Index created'
    >>> index2_output
