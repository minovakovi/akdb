...
int AK_user_check_pass(char *username, char *password)
{
    ...
    while (isspace((unsigned char)*stored_password))
        stored_password++;
    char *end = stored_password + strlen(stored_password) - 1;
    while (end > stored_password && isspace((unsigned char)*end))
        end--;
    *(end + 1) = '\0';
...
TestResult AK_privileges_test()
{
    ...
    if (AK_user_add("user1", "1111", NEW_ID) == USERNAME_TAKEN)
    {
        printf("Test 1. - Fail!\n\n");
    }
    else
