...
void AK_temp_create_table(char *table, AK_header *header, int type_segment)
{
    ...
    // insert object_id
    AK_insert_entry(sys_block, TYPE_INT, &numObjects, objectID);
    // insert table name
...
char *AK_get_attr_name(char *tblName, int index)
{
    ...
    else
    {
        ...
        char *attr_name = strdup(header[index].att_name);
        AK_free(header);
        AK_EPI;
        return attr_name;
    }
...
int AK_table_exist(char *tblName)
{
    ...
    {
        struct list_node *el = AK_get_tuple(a, 1, sys_table);
        if (el != NULL && strcmp(tblName, el->data) == 0)
        {
            exist = 1;
            AK_DeleteAll_L3(&el);
            AK_free(el);
...
{
    struct list_node *currentNode = node;
    int i = 0;
