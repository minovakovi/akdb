...
void AK_selection_op_rename(char *old_table_name, char *new_table_name, struct list_node *expr);
...
struct list_node *AK_find_tuple(int row, int column, int num_attr, table_addresses *addresses, struct list_node *row_root)
{
    ...
    while (addresses->address_from[i] != 0)
    {
        for (j = addresses->address_from[i]; j < addresses->address_to[i]; j += blocks_per_row)
        {
            ...
            if (temp->block->last_tuple_dict_id == 0)
                break;
            while (column >= MAX_ATTRIBUTES)
            { // e.g. 13th column in table is 3rd column in 2nd block of the table
                temp = (AK_mem_block *)AK_get_block(++j);
...
void AK_print_row(int col_len[], struct list_node *row)
{
    ...
    while (el != NULL)
    {
        ...
        case FREE_CHAR:
            ...
            break;
        case TYPE_INT:
            if (el->size < sizeof(int))
            {
                printf(" [ERROR: Bad int size] |");
                break;
            }
            ...
            break;
        case TYPE_FLOAT:
            if (el->size < sizeof(float))
            {
                printf(" [ERROR: Bad float size] |");
                break;
            }
        ...
        case TYPE_VARCHAR:
        default:
            if (el->size <= 0 || el->size > MAX_VARCHAR_LENGTH)
            {
                printf(" [ERROR: Bad string size] |");
                break;
            }
...
int AK_rename(char *old_table_name, char *old_attr, char *new_table_name, char *new_attr)
{
    ...
    if (strcmp(old_attr, new_attr) != 0)
    {
        ...
        for (i = 0; i < MAX_ATTRIBUTES; i++)
        {
            ...
            else if (strcmp(newHeader[i].att_name, "\0") == 0)
    ...
    { // new name is different than old, and old needs to be replaced
