...
size_t AK_fwrite(const void *ptr, size_t size, size_t count, FILE *stream);
size_t AK_fread(void *ptr, size_t size, size_t count, FILE *stream);
 ...
 */
int AK_get_allocation_set(int *allocationSet, int fromWhere, int gaplength, int numRequestedBlocks, AK_allocation_set_mode mode, int target)
{
 ...
 */
void AK_blocktable_dump(int verbosity)
{
  int i;
  AK_PRO;
  printf("Dump of allocation table\n");
...
int AK_print_block(AK_block *block, int num, char *gg, FILE *fpp)
{
  ...
  for (i = 0; i < MAX_ATTRIBUTES; i++)
  {
    ...
    {
      if (tmp >= 5)
        fprintf(fp, "block->header[ ].type = Repeated %d times\n", tmp);
      tmp = 0;
    ...
    for (j = 0; j < MAX_ATT_NAME; j++)
    {
      ...
      {
        if (tmp1 >= 5)
          fprintf(fp, "header[].att_name[] = Repeated %d times\n", tmp1);
        tmp1 = 0;
    ...
    for (j = 0; j < MAX_CONSTRAINTS; j++)
    {
      ...
      {
        if (tmp2 >= 5)
          fprintf(fp, "block->header[].integrity[] = Repeated %d times\n", tmp2);
        tmp2 = 0;
      ...
      for (k = 0; k < MAX_CONSTR_NAME; k++)
      {
        ...
        {
          if (tmp3 >= 5)
            fprintf(fp, "block->header[].constr_name[][]  = Repeated %d times\n", tmp3);
          tmp3 = 0;
      ...
      for (k = 0; k < MAX_CONSTR_CODE; k++)
      {
        ...
        {
          if (tmp4 >= 5)
            fprintf(fp, "block->header[].constr_code[][]  = Repeated %d times\n", tmp4);
          tmp4 = 0;
  ...
  for (i = 0; i < DATA_BLOCK_SIZE; i++)
  {
    ...
    {
      if (tmp >= 5)
        fprintf(fp, "block->tuple_dict[].type = Repeated %d times\n", tmp);
      tmp = 0;
    ...
    {
      if (tmp1 >= 5)
        fprintf(fp, "block->tuple_dict[].address = Repeated %d times\n", tmp1);
      tmp1 = 0;
    ...
    {
      if (tmp2 >= 5)
        fprintf(fp, "block->tuple_dict[].size = Repeated %d times\n", tmp2);
      tmp2 = 0;
  ...
  for (i = 0; i < DATA_BLOCK_SIZE * DATA_ENTRY_SIZE; i++)
  {
    ...
    {
      if (tmp3 >= 5)
        fprintf(fp, "block->data[] = Repeated %d times\n", tmp3);
      tmp3 = 0;
 ...
 */
int AK_copy_header(AK_header *header, int *blockSet, int blockSetSize)
{
  int j = 0;
  int num_blocks = 0;
  int header_att_id = 0;
...
int *AK_get_extent(int start_address, int desired_size, AK_allocation_set_mode *mode, int border, int target, AK_header *header, int gl)
{
  ...
  for (i = 0; i < desired_size; i++)
  {
    ...
    if (i < (desired_size - 1))
      allocationBit->allocationtable[blocknum[i]] = blocknum[i + 1];
  }
...
AK_create_header(char *attribute_name, int type, int integrity, char *constr_name, char *contr_code)
{
  ...
  for (int i = 0; i < MAX_CONSTRAINTS; i++)
  {
    ...
    if (constr_name != NULL)
    {
      strncpy(catalog_header->constr_name[i], constr_name, MAX_CONSTR_NAME);
      catalog_header->constr_name[i][MAX_CONSTR_NAME - 1] = '\0'; // Ensure null termination
    }
    else
    {
      catalog_header->constr_name[i][0] = '\0'; // Assign empty string if NULL
    }

    if (contr_code != NULL)
    {
      strncpy(catalog_header->constr_code[i], contr_code, MAX_CONSTR_CODE);
      catalog_header->constr_code[i][MAX_CONSTR_CODE - 1] = '\0'; // Ensure null termination
    }
    else
    {
      catalog_header->constr_code[i][0] = '\0'; // Assign empty string if NULL
    }
  }
